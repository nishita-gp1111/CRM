...
export async function POST(request: Request) {
  try {
    ...
    const version = await prisma.formVersion.findFirst({
      where: { id: input.versionId, formId: form.id, organizationId: context.organization.id },
    });
    if (!version) return NextResponse.json({ message: "バージョンが見つかりません。" }, { status: 404 });
    const draftSchema = normalizeAppointmentFormSchema(version.fieldSchema);
